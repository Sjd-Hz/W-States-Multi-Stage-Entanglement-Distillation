 ...
 %  via CNOT teleportation
    ...
    %% State Preparation
    % Initial state (|010⟩ + |100⟩)/√2
    psi3 = (1/sqrt(2)) * (kron(ket0, kron(ket1, ket0)) + ...
            (kron(ket1, kron(ket0, ket0))));
    rho3 = psi3 * psi3'
    
    % Apply Hadamard to qubit 1
    Had3 = kron(Id2,kron(Id2, Had));
    rho_had = Had3 * rho3 * Had3';
    
    % Create Bell state (|01⟩ + |10⟩)/√2 between qubits B2 and B3
    bell = (1/sqrt(2)) * (kron(ket0, ket1) + kron(ket1, ket0));
    rho_bell = bell * bell';
    
    % Combine systems (now 5 qubits: 1,2,3,B2,B3)
