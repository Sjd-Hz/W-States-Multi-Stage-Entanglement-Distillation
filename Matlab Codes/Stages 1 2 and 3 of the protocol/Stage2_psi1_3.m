 %% (psi_1,psi_3) 
 % Transforming the GHZ-like state: |ψ⟩ = (|001⟩ + |100⟩)/√2 to
 %  W-like state: |ψ⟩ = (|001⟩ + |010⟩+ |100⟩+ |111⟩)/2
 %  via CNOT teleportation with 
 %\psi_3: bell between qubits 1 and 2, so CNOT between 1 and 2
    ...
    Had2 = kron(Had,kron(Id2, Id2));
    rho_had = Had2 * rho1 * Had2';
    
    % Create Bell state (|01⟩ + |10⟩)/√2 between qubits B1 and B2
    ...
    %% CNOT Operations
    % CNOT: qubit 1 controls B2 (qubit 4)
    CNOT_1to4 = CNOTn(5,1,4);
    
    % CNOT: B2 (qubit 5) controls qubit 2
    CNOT_5to2 = CNOTn(5,5,2);
    
    % Apply both CNOTs
    rho_after_CNOTs = CNOT_5to2 * CNOT_1to4 * rho_total * CNOT_1to4' * CNOT_5to2';
